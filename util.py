...
z_to_z_str_dict = {v: k for k, v in z_str_to_z_dict.items()}
...
def print_x_out_of_y(s, x, y, finish=False, width=None):
    ...
    if finish:
        ...
        _print(
            f"{bcolors.OKBLUE}{a:<{width}}{bcolors.ENDC}{s}{bcolors.FLASH}...{bcolors.ENDC}",
            end="\r",
        )
...
def print_x_out_of_y_eta(s, x, y, eta, finish=False, width=None):
    ...
    if finish:
        eta = f"({eta:.1f}s)"
        _print(
            f"{bcolors.OKGREEN}{a:<{width}}{bcolors.ENDC}{s:<70}{bcolors.WHITE}{eta:>10}{bcolors.ENDC}"
        )
    else:
        rem_len = 70 - len(s)
        eta = f"(ETA {eta:.1f}s)"
        _print(
            f"{bcolors.OKBLUE}{a:<{width}}{bcolors.ENDC}{s}"
            + f"{bcolors.FLASH}{'...':<{rem_len}}{bcolors.ENDC}{bcolors.WHITE}{eta:>10}{bcolors.ENDC}",
            end="\r",
        )
...
def print_percent(s, x, y, finish=False):
    ...
    if finish:
        _print(
            f"{bcolors.OKGREEN}{'(100%)':<{UI_COLUMN_WIDTH}}"
            f"{bcolors.ENDC}{s}       "
        )
    else:
        a = f"({p}%)"
        _print(
            f"{bcolors.OKBLUE}{a:<{UI_COLUMN_WIDTH}}{bcolors.ENDC}{s}"
            f"{bcolors.FLASH}...{bcolors.ENDC}",
            end="\r",
        )
...
def print_ongoing_process(s, finish=False, time=None):
    if finish:
        if time is not None:
            ...
            time_s = ""
        _print(
            f"{bcolors.OKGREEN}{'(DONE)':<{UI_COLUMN_WIDTH}}{bcolors.ENDC}"
            + f"{s:<70}{bcolors.WHITE}{time_s:>10}{bcolors.ENDC}"
        )
    else:
        _print(
            f"{bcolors.OKBLUE}{'(WAIT)':<{UI_COLUMN_WIDTH}}{bcolors.ENDC}"
            + f"{s}{bcolors.FLASH}...{bcolors.ENDC}{' '*20}",
            end="\r",
        )
...
def print_cluster_scheme(scheme, para):
    ...
    for i in scheme:
        info = para[i]
        _print(
            f"{i:<7}{info.get('n_clusters','N/A'):<10}{info.get('type','N/A').replace('func:','')}"
        )
...
def find_function(name, para, SEARCH_MODULES):
    ...
    print_error(
        "No callable function called {} found in find_function. Aborted.".format(name)
    )
...
def generate_custom_args(self, args):
    ...
    if not (isinstance(args, list) or isinstance(args, tuple)):
        print_error(
            "Invalid variable type for args in generate_custom_args. Must be tuple or list. Aborted"
        )
...
def generate_custom_kwargs(self, kwargs):
    ...
    if not (isinstance(kwargs, dict)):
        print_error(
            "Invalid variable type for kwargs in generate_custom_kwargs. Must be dict."
        )
...
def run_custom_file(db, file, args):
    ...
    if ext == ".py":
        ...
        call = [path] + [
            str(x) for x in args
        ]  # bash script needs it all to be strings ofc
        subprocess.call(call)
    else:
        print_error(
            "File type {} not recognised in run_custom_file. Aborted.".format(ext)
        )
...
def find_valid_path(path):
    ...
    while os.path.exists(path):
        ...
        if n > 1000:
            print_error(
                "Broke out of 'find_valid_path' after {} iterations (current path: {}). How?".format(
                    n, path
                )
            )
    return path
...
def color_interp(color_steps, values):
    ...
    x_c = [x_min + delta * j / (n_color_steps - 1) for j in range(n_color_steps)]
