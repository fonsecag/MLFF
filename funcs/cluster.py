...
def smallest_max_distance_euclidean(sample):
    ...
    for i in range(len(clusters)):
        g[i] = np.max(
            np.sum(np.square(clusters[i] - sample), 1)
...
def agglomerative_clustering(self, indices, scheme_index):
    ...
    if self.call_para("clusters", scheme_index, "custom"):
        ...
        M = self.call_para(
            "clusters", scheme_index, "distance_matrix_function", args=[data_init]
        )
        ...
        print_ongoing_process("Computing tree")
        cinit_labels = AgglomerativeClustering(n_clusters=n_clusters).fit_predict(
            data_init
        )
    ...
    g_cluster_data = [
        mp.RawArray("d", cluster_data[i].ravel()) for i in range(len(cluster_data))
    ]
...
def cluster_do(self, scheme, init_ind=None):
    ...
    for i in range(1, len(scheme)):
        ...
        cl_ind_new = []
        for cl in cl_ind:
            cl_cl_ind = self.call_para(
                "clusters", index, "type", args=[self, cl, index]
            )
            for j in cl_cl_ind:
                cl_ind_new.append(j)
...
def cluster_fraction_min(self, ind, frac, mini):
    a = []
    for x in ind:
        N = min(len(x), int(min(len(x) * frac, mini)))
        a.append(np.random.choice(x, N, replace=False))
