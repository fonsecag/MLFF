...
class ClusterErrorHandler(ClusterHandler):
    ...
    def run_command(self):
        ...
        self.train_load_model(
            os.path.join(self.storage_dir, "model"), self.args["init"], None
        )
    ...
    def train_load_model(self, model_path, indices, old_model_path=None):
        ...
        if self.get_para("train_models", "model_info_func") is not None:
            self.call_para("train_models", "model_info_func", args=[self])
    ...
    def predict_indices(self, ind, predict_index):
        ...
        pred = self.call_para(
            "predict_error", "predicts", predict_index, "predict_func", args=[self, var]
        )
        return pred
    ...
    def calculate_errors(self, extended=False, sample_wise=False):
        ...
        main_error_index = self.call_para("predict_error", "main_error_index")
        ...
        for error_index in l_errors:
            ...
            pred_index = self.call_para("predict_error", error_index, "predicts_index")

            pred = self.predicts[pred_index]

            err_key = self.call_para("predict_error", error_index, "save_key")

            by_cluster = self.call_para("predict_error", error_index, "by_cluster")
            ...
            if by_cluster or (error_index == main_error_index):
                if not by_cluster:
                    print_warning(
                        f"main_error_index should be tied to by_"
    ...
    def plot_cluster_error(self):
        ...
        for i in range(len(self.para["predict_error"])):
            ...
            if not hasattr(err, "__len__"):
                print_warning(
                    f"Error key {key} is set to by_cluster but error"
                ...
                print_warning(
                    f"Error key {key} is has length {len(err)} but "
            ...
            if len(color_steps) < 2:
                print_warning(
                    f"Parameter color_steps should be >1," f" is {len(color_steps)}."
                )
            ...
            ax.bar(x, err, color=bar_color, width=self.get_graph_para("bar_width", i))
            ...
            if self.get_graph_para("include_population", i):
                ...
                ax.text(
                    0,
                    y,
                    self.get_graph_para("population_text", i),
                    fontsize=fontsize,
                    color=color,
            ...
            ax.set_title(key, fontsize=self.get_graph_para("title_fontsize", i))
